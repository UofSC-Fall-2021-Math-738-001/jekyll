...
\begin{tikzcd}
\operatorname{Hom}(P,I) & \operatorame{Hom}(A,I) \ar[l] \\
...
with 
$$
    (P \otimes_R Q)^i = \bigooplus_{a+b=i} P^a \otimes_R Q^b
$$
and 
$$
...
- $F$ is flat.
- $\operatarname{Tor}_i^R(F,-) = 0$ for $i > 0$.
- $\operatorname{Tor}_1^R(F,-) = 0$. 

{% eqn %}
Assume that $F$ is flat. And take $P \doubleheadedarrow M$ with $M$ projective. Then, 
...
$$
    \operatorname{Tor}_2^R(F,M) \cong \operatorame{Tor}_1^R(F,K)  
$$
...
If $\operatarname{Tor}_i^R(F,-) = 0$ for $i > 0$, then certainly $\operatorname{Tor}_1^R(F,-) = 0$. 
...
$$
     0 \to F \otimes_R C \to F \otimes_R B \to F \otimes_A \to 0 
$$
is exact. 
{% endeqn %}
